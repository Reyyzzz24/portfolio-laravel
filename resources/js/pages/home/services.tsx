...
const Services = ({ content }: { content: PortfolioContent['services'] | any[] | null }) => {
    ...
    return (
        <section id="services" className="bg-gray-50 py-24 transition-colors dark:bg-gray-800/50">
            <div className="container mx-auto max-w-7xl px-6 md:px-12">
                <div className="mb-16 text-center">
                    <h3 className="text-sm font-semibold uppercase tracking-widest text-blue-600">
                        {contentObj.eyebrow}
                    </h3>
                    <h1 className="mt-2 text-4xl font-bold md:text-5xl">{contentObj.title}</h1>
                    <div className="mx-auto mt-6 h-1.5 w-20 rounded-full bg-blue-600"></div>
                </div>
                ...
                >
                    {contentObj.items.map((service: any) => {
                        ...
                        const colorClass = isHexColor ? '' : (serviceColors[service.color] ?? serviceColors.blue);

                        return (
                            <motion.div
                                key={service.title}
                                variants={itemVariants}
                                whileHover={{ y: -15, scale: 1.02 }} // Efek Pop Out
